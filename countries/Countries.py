...
class Countries:
    ...
    def get_all_papers(self):
        ...
        for country in self.lst_of_countries:
            to_return = self.pc.get_papers_ored_by_countries(to_return, self.pc.get_papers_from_country(country))
        return to_return
    ...
    def get_themes_distribution(self):
        ...
        for key in dict_themes.keys():
            dict_themes.update({key: len(self.pc.get_papers_anded_by_id(self.all_papers,
    ...
    def get_watercode_distribution(self):
        ...
        water_papers = self.pc.get_papers_anded_by_id(self.all_papers, self.pc.get_papers_from_primary_theme('water'))
    ...
    def get_watercode_distribution_secondary(self):
        dict_watercode = {'green': 0, 'blue': 0, 'brown': 0}
        water_papers = self.pc.get_papers_anded_by_id(self.all_papers, self.pc.get_papers_from_secondary_theme('water'))
        for paper in water_papers:
            key = self.w.get_watercode_from_paper(paper)
            dict_watercode.update({key.get('WATER_CODE'): dict_watercode.get(key.get('WATER_CODE')) + 1})
        return dict_watercode
